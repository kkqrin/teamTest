...
public class PostViewFrmServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * int postNo = Integer.parseInt(request.getParameter("postNo")); PostService
		 * service = new PostService(); Post p = service.selectOnePost(postNo); if(p ==
		 * null) { request.setAttribute("title", "성공"); request.setAttribute("msg",
		 * "삭제된 게시물 입니다."); request.setAttribute("icon", "error");
		 * request.setAttribute("loc", "/postListNotRead.do?reqPage=1&memberId=admin");
		 * RequestDispatcher view =
		 * request.getRequestDispatcher("/WEB-INF/views/common/msg.jsp");
		 * view.forward(request, response); }else { request.setAttribute("p", p);
		 * RequestDispatcher view =
		 * request.getRequestDispatcher("/WEB-INF/views/post/postViewFrm.jsp");
